...
> 📌 **参照:** [OpenAI Platform — API Keys](https://platform.openai.com/api-keys)
> 画面で確認する内容: OpenAI Platformにログイン後、左メニューの「API keys」を開くと、既存のキー一覧と「Create new secret key」ボタンが表示されます。ボタンをクリックしてキー名を入力し「Create secret key」をクリックすると、`sk-` で始まるAPIキーが表示されます。**このキーは一度しか表示されません。** 必ずコピーして安全な場所に保管してください。
